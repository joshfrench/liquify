      ...
      class RenderTags
        ...
        private
          def render_content_with_page(tag_content, page)
            ...
            request.params[:sample_param] = 'data'
            request.request_uri = @request_uri || page.url
            request.host = @host || test_host
            ActionController::Base.relative_url_root = @relative_root
            page.response = ActionController::TestResponse.new
            Liquid::Template.parse(tag_content).render({'page' => Liquify::PageDrop.new(page)}, :registers => {'request' => request})
          end
